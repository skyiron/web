<template>
  ...
  <div v-else>
    <oc-table :fields="fields" :data="slots" class="slots-table">
      <template #type="{ item }">
        <code> {{ item.type }}</code>
      </template>
...
const { loadSlots } = useLoadMetaData()
...
const slots: Ref<Prop[]> = ref([])
const isLoading = ref(true)

onMounted(async () => {
  slots.value = await loadSlots(component || unref(componentName))
